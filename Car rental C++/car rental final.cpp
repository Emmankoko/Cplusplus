...
class rent : public customer // inhereted class from customer class
{
	...
	cout << "\t\t\t\tPlease Enter your full Name: "; //taking data from the user
    getline(cin >> ws, customername); // reading whitespaces which sepererate your names
    cout<<endl;
    do
    {
...
void showrent()
    ...
    cout << "\t\t	///////////////////////////////////////////////////////////"<<endl;
    cout << "\t\t	| Invoice No. :"<<"------------------|"<<setw(20)<<"#Cnb81353"<<" |"<<endl;
    cout << "\t\t	| Customer Name:"<<"-----------------|"<<setw(20)<<customername<<" |"<<endl;
    cout << "\t\t	| Car Model :"<<"--------------------|"<<setw(20)<<carmodel<<" |"<<endl;
    cout << "\t\t	| Car No. :"<<"----------------------|"<<setw(20)<<carnumber<<" |"<<endl;
    cout << "\t\t	| Number of days :"<<"---------------|"<<setw(20)<<days<<" |"<<endl;
    cout << "\t\t	| Your Rental Amount is :"<<"--------|"<<setw(20)<<rentalfee<<" |"<<endl;
    cout << "\t\t	| Caution Money :"<<"----------------|"<<setw(20)<<"0"<<" |"<<endl;
    cout << "\t\t	| Advanced :"<<"---------------------|"<<setw(20)<<"0"<<" |"<<endl;
    cout << "\t\t	 ________________________________________________________"<<endl;
    cout <<"\n";
    cout << "\t\t	| Total Rental Amount is :"<<"-------|"<<setw(20)<<rentalfee<<" |"<<endl;
    cout << "\t\t	 ________________________________________________________"<<endl;
...
class welcome //welcome class
{
 ...
 ifstream in("welcome.txt"); //displaying welcome ASCII image text on output screen fn1353
  ...
  Sleep(2000); //function which waits for (n) seconds
...
public:
   ...
   cout << "\t\t\t\t\tEnter email: ";
        ...
        char charrr; // creating your password
        char charrr_confirm;//confirm password
        cin >> email;
        cout<<endl;
    cout << "\t\t\t\t\tEnter password: ";
        charrr = _getch();
     ...
     cout << "\t\t\t\t\tconfirm your password: ";
         charrr_confirm = _getch();
        while(charrr_confirm != 13)
        {
            if(charrr_confirm == 010)
            {
                password_confirm.pop_back();
            }
            else {
                password_confirm.push_back(charrr_confirm);
            }
            charrr_confirm = _getch();
        ...
        {
            cout << "\t\t\t\t\tconfirming your password: "<<endl;
            Sleep(2000);
    ...
    string passw() //function to return confirmed password since it's a private field
    {
...
class Login : public sign_up
{
   ...
   while(ch != 13){//character 13 is enter
    if(ch == 010)// character 010 is for backspace
        pass.pop_back(); // remove the last character
    else{
        pass.push_back(ch);//user enters password invinsibly which improves security
    }
    ch = _getch();
   }


   if(pass == passw() && login_email == email){
      cout << "\n\n\n\t\t\t\t\t\tAccess Granted! \n";
      system("PAUSE");
      system ("CLS");
   }else{
